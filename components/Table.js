...
export default function MyTable (props) {
        ...
        if (typeof columns == "object")
            for (let column of columns) {
                ...
                if (column.type == "trend")
                    ...
                    render = (text) => (
                        <Link key={text.id} href={(text) ? `/wb/${column.link}/` + text.id : ""}>
                            {(typeof text == "object" && text.hasOwnProperty("title")) ? text.title : ""}
                        </Link>
                    );
            ...
            if (columns[i].type == "title")
                data[i].render = (text, record, index) => {
                    const popoverContent = text.src && (
                        <Space>
                            <Image src={text.src} preview={false} height="50vh"/>
                    ...
                    return (
                        <div key={text.id} style={{ marginTop: -8 }}>
                            {text.src && <Popover content={popoverContent}
                                                  placement="right" title={text.title}>
                                <Image src={text.src}
                                       preview={{ src: text.src, mask: <EyeOutlined/> }} height={40}/>
                            </Popover>}
    ...
    const [columnsDataDraggable, setColumnsDataDraggable] = useState(columnsData);
    ...
    const dragProps = {
        onDragEnd(fromIndex, toIndex) {
            ...
            // console.log(columns);
            setColumnsDataDraggable((prevColumns) => {
        ...
        nodeSelector: "th",
        ignoreSelector: "th.ant-table-selection-column"
    };
