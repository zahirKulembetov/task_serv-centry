...
export default function getColumnSearchProps (type, setUrl, bUrl, name, dataIndex, setFilters) {
    ...
    const handleSearch = (selectedKeys, confirm, dataIndex) => {
        ...
        if (type != 1)
            setUrl(`/${type}/by_${dataIndex}?${dataIndex}=${selectText}`);
    };
        ...
        onFilter: (type == 1) ?
            (value, record) => {
                if (typeof record[dataIndex] == "object") {
                    if ("name" in record[dataIndex])
                        return record[dataIndex]["name"].toString().toLowerCase().indexOf(value.toLowerCase()) > -1;
                } else
